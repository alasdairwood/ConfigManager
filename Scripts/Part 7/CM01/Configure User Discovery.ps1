<#
# Configure User Discovery, 2019/8/27 Niall Brady, for more info see https://www.windows-noob.com/forums/topic/16614-how-can-i-install-system-center-configuration-manager-current-branch-version-1902-on-windows-server-2019-with-sql-server-2017-part-1/
#
# This script:            configures User Discovery
# Before running:         Edit the variables as necessary (lines 28-44).
...
$OUDistinguishedName ="OU=Users,OU=aiwtech,DC=aiwtech,DC=co,DC=uk"
$OUParentName = "Active Directory User Discovery - $SiteCode"
...
write-host "ActiveDirectoryUserDiscovery" -ForegroundColor Green -NoNewLine

Set-CMDiscoveryMethod -ActiveDirectoryUserDiscovery -SiteCode $SiteCode -Enabled $Enabled -AddActiveDirectoryContainer $LdapLocation -AddAdditionalAttribute $AddAdditionalAttribute -PollingSchedule $Schedule -EnableDeltaDiscovery $EnableDeltaDiscovery -DeltaDiscoveryMins $DeltaDiscoveryMins #-Verbose
